...
from utils.misc.azure_utils import load_file_from_blob, upload_checkpoint_files, upload_checkpoint_file
...
def prepare(cfg, use_arg_parser=True):
    ...
    cfg['OUTPUT_MODEL_NAME'] = "faster_rcnn_eval_{}_{}.model".format(cfg["MODEL"].BASE_MODEL, "e2e" if cfg["CNTK"].TRAIN_E2E else "4stage")
    cfg['MODEL_PATH'] = os.path.join(os.environ['AZUREML_NATIVE_SHARE_DIRECTORY'], "output", cfg['OUTPUT_MODEL_NAME'])
    cfg['BASE_MODEL_PATH'] = os.path.join(os.environ['AZUREML_NATIVE_SHARE_DIRECTORY'], "base_model")
    ...
    cfg['BASE_MODEL_PATH'] = cfg['BASE_MODEL_PATH'] + '_' + cfg["MODEL"].BASE_MODEL_FILE
...
def train_faster_rcnn(cfg):
    ...
    if os.path.exists(model_path) and cfg["CNTK"].MAKE_MODE:
        ...
        upload_checkpoint_file(os.path.join(os.environ['AZUREML_NATIVE_SHARE_DIRECTORY'], 'output', cfg['OUTPUT_MODEL_NAME']), cfg['OUTPUT_MODEL_NAME'], add_timestamp=True)
...
def train_model(image_input, roi_input, dims_input, loss, pred_error,
    ...
    for epoch in range(epochs_to_train):       # loop over epochs
        ...
        while sample_count < cfg["DATA"].NUM_TRAIN_IMAGES:  # loop over minibatches in the epoch
            ...
            if sample_count % training_progress_output_freq == 0:
    ...
    # log the losses and errors for Azure ML Workbench
